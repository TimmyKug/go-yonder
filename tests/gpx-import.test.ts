...
describe("importGpxText", () => {
  ...
  const ingest = (samples: readonly NormalizedLocationSample[]) => service.ingest(samples);
  ...
  it("adds points as imported history and unlocks their tiles", async () => {
    const result = await importGpxText(syntheticWalk(), { database, ingest });
  ...
  it("adds nothing when the same file is imported again", async () => {
    await importGpxText(syntheticWalk(), { database, ingest });
    const again = await importGpxText(syntheticWalk(), { database, ingest });
  ...
  it("does not duplicate live points when Yonder's own export is imported", async () => {
    ...
    const result = await importGpxText(exported, { database, ingest });
...
<extensions><yonder:accuracy>400</yonder:accuracy></extensions></trkpt></trkseg></trk></gpx>`;
    const result = await importGpxText(gpx, { database, ingest });
