...
describe("additive backup import", () => {
  it("merges cells and recorded locations without replacing local data or duplicating samples", async () => {
    ...
    expect(await mergeBackupUnlocks(source, target)).toEqual({ addedCount: 1, totalCount: 2,
      addedSampleCount: 1, totalSampleCount: 2, skippedSampleCount: 0 });
    ...
    expect(await mergeBackupUnlocks(source, target)).toEqual({ addedCount: 0, totalCount: 2,
      addedSampleCount: 0, totalSampleCount: 2, skippedSampleCount: 0 });
  ...
  it("skips unreadable samples, recomputes fingerprints and still merges everything else", async () => {
    const source = await database();
    const target = await database();
    await add(source, b);
    ...
    // Written by another tool: valid values under a fingerprint Yonder does not use.
    await source.run(`INSERT INTO location_samples
      (source, recorded_at_ms, latitude, longitude, horizontal_accuracy_m, fingerprint)
      VALUES ('live-background', ?, 3, 0, 400, 'other-fingerprint-format')`, [Date.UTC(2026, 2, 1)]);
    // A time outside JavaScript's date range cannot be restored.
    await source.run(`INSERT INTO location_samples
    ...
    expect(await mergeBackupUnlocks(source, target)).toEqual({ addedCount: 1, totalCount: 1,
      addedSampleCount: 257, totalSampleCount: 258, skippedSampleCount: 1 });
    const restored = await target.first<{ fingerprint: string; horizontal_accuracy_m: number }>(
      "SELECT fingerprint, horizontal_accuracy_m FROM location_samples WHERE latitude = 3");
    // Accepted when recorded, so the current live accuracy limit does not apply.
    expect(restored?.horizontal_accuracy_m).toBe(400);
    expect(restored?.fingerprint).toMatch(/^location-sample-v1:/);
    expect(await mergeBackupUnlocks(source, target)).toMatchObject({ addedSampleCount: 0, skippedSampleCount: 1 });
  });
